...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE danh_gia SET san_pham_id='$san_pham_id', nguoi_dung_id='$nguoi_dung_id', 
            diem_danh_gia='$diem_danh_gia', noi_dung='$noi_dung' WHERE id='$id'";
    
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Cập nhật đánh giá thành công!'); window.location='manage_reviews.php';</script>";
    } else {
        echo "<script>alert('Lỗi khi cập nhật đánh giá: " . mysqli_error($conn) . "');</script>";
    }
}
...
<html lang="en">
<head>
  <title>Sửa đánh giá | Quản trị Admin</title>
  <meta charset="utf-8">
  ...
  <main class="app-content">
    <div class="app-title">
      <ul class="app-breadcrumb breadcrumb">
        ...
        <li class="breadcrumb-item active"><b>Sửa đánh giá</b></li>
      </ul>
    </div>
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          <h3 class="tile-title">Sửa đánh giá #<?php echo $id; ?></h3>
          <div class="tile-body">
            <form method="POST">
              <div class="form-group">
                ...
                <select name="san_pham_id" class="form-control" required>
                  ...
                  <?php while ($row = mysqli_fetch_assoc($result_products)) { ?>
                    <option value="<?php echo $row['id']; ?>" <?php echo $row['id'] == $review['san_pham_id'] ? 'selected' : ''; ?>>
              ...
              <div class="form-group">
                ...
                <select name="nguoi_dung_id" class="form-control" required>
                  ...
                  <?php while ($row = mysqli_fetch_assoc($result_users)) { ?>
                    <option value="<?php echo $row['id']; ?>" <?php echo $row['id'] == $review['nguoi_dung_id'] ? 'selected' : ''; ?>>
              ...
              <div class="form-group">
                ...
                <input type="number" name="diem_danh_gia" class="form-control" min="1" max="5" value="<?php echo $review['diem_danh_gia']; ?>" required>
              </div>
              <div class="form-group">
                <label>Nội dung:</label>
                <textarea name="noi_dung" class="form-control" rows="5"><?php echo htmlspecialchars($review['noi_dung']); ?></textarea>
              </div>
