...
$sql = "SELECT * FROM separator_media WHERE id = '$id'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE separator_media SET media_path='$media_path', type='$type', mo_ta='$mo_ta', thu_tu='$thu_tu' WHERE id='$id'";
    if (mysqli_query($conn, $sql)) {
        header("Location: manage_separator_media.php");
        exit();
    } else {
        echo "<script>swal('Lỗi!', 'Không thể cập nhật Separator Media: " . mysqli_error($conn) . "', 'error');</script>";
    }
}
?>
...
<head>
  <title>Sửa Separator Media | Quản trị Admin</title>
  <meta charset="utf-8">
  ...
  <main class="app-content">
    <div class="app-title">
      <ul class="app-breadcrumb breadcrumb">
        ...
        <li class="breadcrumb-item active"><a href="#">Sửa Separator Media</a></li>
      </ul>
      <div id="clock"></div>
    </div>
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          <h3 class="tile-title">Sửa Separator Media</h3>
          <div class="tile-body">
            <form method="POST" class="row">
              <div class="form-group col-md-6">
                <label class="control-label">Đường dẫn Media</label>
                <input class="form-control" type="text" name="media_path" value="<?php echo htmlspecialchars($row['media_path']); ?>" required>
              </div>
              <div class="form-group col-md-6">
                <label class="control-label">Loại</label>
                <select class="form-control" name="type" required>
                  <option value="image" <?php if ($row['type'] == 'image') echo 'selected'; ?>>Hình ảnh</option>
                  <option value="video" <?php if ($row['type'] == 'video') echo 'selected'; ?>>Video</option>
                  <option value="none" <?php if ($row['type'] == 'none') echo 'selected'; ?>>Không</option>
                </select>
              </div>
              <div class="form-group col-md-12">
                <label class="control-label">Mô tả</label>
                <textarea class="form-control" name="mo_ta" rows="4"><?php echo htmlspecialchars($row['mo_ta']); ?></textarea>
              </div>
              <div class="form-group col-md-6">
                <label class="control-label">Thứ tự</label>
                <input class="form-control" type="number" name="thu_tu" value="<?php echo $row['thu_tu']; ?>" required>
              </div>
              <div class="form-group col-md-12">
