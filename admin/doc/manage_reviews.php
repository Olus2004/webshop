...
if (isset($_GET['delete'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        echo "<script>swal('Lỗi khi xóa đánh giá!');</script>";
    }
}

// Lấy danh sách đánh giá
$sql = "SELECT dg.id, sp.ten_san_pham, nd.ten_dang_nhap, dg.diem_danh_gia, dg.noi_dung, dg.ngay_danh_gia
        FROM danh_gia dg
...
<head>
  <title>Danh sách đánh giá | Quản trị Admin</title>
  <meta charset="utf-8">
        ...
        <div class="tile">
          <div class="tile-body">
            <div class="row element-button">
              <div class="col-sm-2">
                <a class="btn btn-add btn-sm" href="form-add-review.php" title="Thêm"><i class="fas fa-plus"></i>
                  Thêm đánh giá mới</a>
              </div>
              <div class="col-sm-2">
              ...
              <tbody>
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                  <tr>
                    ...
                    <td><?php echo $row['diem_danh_gia']; ?>/5</td>
                    <td><?php echo $row['noi_dung'] ? htmlspecialchars($row['noi_dung']) : 'Không có nội dung'; ?></td>
                    <td><?php echo date('d/m/Y H:i:s', strtotime($row['ngay_danh_gia'])); ?></td>
                    <td>
                      ...
                      <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" onclick="location.href='edit-review.php?id=<?php echo $row['id']; ?>'"><i class="fa fa-edit"></i></button>
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
