...
// Xử lý xóa separator_media
if (isset($_GET['delete'])) {
    $id = mysqli_real_escape_string($conn, $_GET['delete']);
    $sql = "DELETE FROM separator_media WHERE id = '$id'";
    if (mysqli_query($conn, $sql)) {
        echo "<script>swal('Đã xóa thành công!');</script>";
    } else {
        echo "<script>swal('Lỗi khi xóa dữ liệu!');</script>";
    }
}

// Lấy danh sách separator_media
$sql = "SELECT * FROM separator_media ORDER BY thu_tu ASC";
...
<head>
  <title>Danh sách Separator Media | Quản trị Admin</title>
  <meta charset="utf-8">
...
<body onload="time()" class="app sidebar-mini rtl">
  ...
  <aside class="app-sidebar">
    ...
    <ul class="app-menu">
      ...
      <li><a class="app-menu__item active" href="manage_separator_media.php"><i class='app-menu__icon bx bx-image'></i><span class="app-menu__label">Separator Media</span></a></li>
    </ul>
  </aside>
  <main class="app-content">
    <div class="app-title">
      <ul class="app-breadcrumb breadcrumb side">
        <li class="breadcrumb-item active"><a href="#"><b>Danh sách Separator Media</b></a></li>
      </ul>
      <div id="clock"></div>
    </div>
    <div class="row">
        ...
        <div class="tile">
          <div class="tile-body">
            <div class="row element-button">
              <div class="col-sm-2">
                <a class="btn btn-add btn-sm" href="form-add-separator-media.php" title="Thêm"><i class="fas fa-plus"></i>
                  Thêm Separator Media</a>
              </div>
              <div class="col-sm-2">
              ...
              <tbody>
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                  <tr>
                    ...
                    <td><?php echo htmlspecialchars($row['media_path'] ?: 'Không có'); ?></td>
                    <td>
                      <?php 
                        if ($row['type'] == 'image') {
                          echo '<span class="badge bg-success">Hình ảnh</span>';
                        } elseif ($row['type'] == 'video') {
                          echo '<span class="badge bg-info">Video</span>';
                        } else {
                          echo '<span class="badge bg-secondary">Không</span>';
                        }
                      ?>
                    </td>
                    <td><?php echo htmlspecialchars($row['mo_ta'] ?: 'Không có mô tả'); ?></td>
                    ...
                    <td>
                      ...
                      <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" onclick="location.href='edit-separator-media.php?id=<?php echo $row['id']; ?>'"><i class="fa fa-edit"></i></button>
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
  ...
  <script>
    jQuery(function () {
      jQuery(".trash").click(function () {
        ...
        swal({
          title: "Cảnh báo",
          text: "Bạn có chắc chắn muốn xóa Separator Media này?",
